...
sub check_archive {
	...
	if (exists $actual{$name}) {
	    if (file_dir ($actual{$name}) && !file_dir ($expected{$name})) {
		print "$name is a directory but should be an ordinary file.\n";
		$errors++;
	    } elsif (!file_dir ($actual{$name}) && file_dir ($expected{$name})) {
...
sub normalize_fs {
	...
	my ($value) = $fs{$name};
	next if file_dir ($value) || ref ($value) ne '';
