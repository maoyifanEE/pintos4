...
int write(int fd, const void *buffer, unsigned size)
{
    ...
    {
        curfile = cur->fdt[fd];
        if(inode_file_dir(curfile->inode)){
            goto done;
        ...
        if (curfile->write_deny)
        {
            file_write_deny(curfile);
        }
...
bool chdir(const char *dir){
    ...
    return fliesys_children_dir(dir);
}
...
bool readdir(int fd, char *name){
    ...
    if(!inode_file_dir(file->inode)){
        return success;
    }
   // struct dir * dir = dir_open(file);
    success = dir_read((struct dir*) file,name);
    //dir_close(dir);
...
bool isdir(int fd){
    ...
    return inode_file_dir(file->inode);
